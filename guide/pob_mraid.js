...
(function() {
    ...
    var EVENTS = {
        ...
        APP_INSTALL_STATUS  : "appInstallStatus", // Custom event for app install status, not defined in MRAID specs, added SDK v4.10.0
    };
    ...
    mraidService.setLocation = function(props) {
       ...
       * @since OpenWrap SDK v4.10.0
       * @param {*} app, the bundle id of the app to check for installation status.
       */
      mraidService.requestAppInstallStatus = function (app) {
        ...
        mraidService._currentAppInstallRequest = app;
        ...
        mraidService._checkAppInstallStatusTimeout = setTimeout(() => {
          mraidService.onAppInstallStatusReceived(app, -1);
        }, 1000);
        mraidService.nativeInvoke("requestAppInstallStatus", { app: app });
      };

      /**
       * Called when the app install status is received from the native layer.
       * @since OpenWrap SDK v4.10.0
       * @param {*} app, the bundle id of the app to check for installation
       ...
       */
      mraidService.onAppInstallStatusReceived = function (app, appInstallStatus) {
        if ( mraidService._isAppInstallRequestInProgress &&
          mraidService._currentAppInstallRequest === app ) {
          ...
          const APP_INSTALL_STATUS_ERROR_MESSAGE = "Can not fire APP_INSTALL_STATUS event for app " + app + " as app install status is not requested.";
    ...
    var mraid = {
         ...
         * This will help creative to check if the app with a specific bundle id is installed on the device.
         ...
         *   console.log(`App ${data.app} is ${data.appInstallStatus === 1 ? 'installed' : 'not installed'}`);
         * });
         *
         * Step 2: call the appInstallStatus method
         * mraid.requestAppInstallStatus("com.example.app");
         *
         * @since OpenWrap SDK v4.10.0
         * @param {*} app, the bundle id of Android App to check for installation status
         */
        requestAppInstallStatus: function (app) {
          mraidService.requestAppInstallStatus(app);
        },
    };
